...
- We'll totally create "2" instances.
 ...
 1. Create EC2 that is installed httpd user data in default VPC named "Desginer"
```bash
Region: "N.Virginia"
VPC: Default VPC
Subnet: PublicA
...
### STEP 3: Create Static WebSite Hosting :
 ...
 1. Create Static WebSite Hosting-1/ "www.[your sub-domain name].net"
  ...
  - Upload Files named "index.html" and "eagle.jpg" in "s3.bucket.www" folder
...
cd /usr/share/nginx/html
chmode -R 777 /usr/share/nginx/html
echo "<center><h1>Hello World from <h2>private: $(hostname -f) and <h2>public: $(curl http://169.254.169.254/latest/meta-data/public-ipv4)</center>" >> /usr/share/nginx/html/index.html 
systemctl enable nginx
systemctl start nginx
...
- Launch 2 Instances on 2 different AZs from Launch Template
