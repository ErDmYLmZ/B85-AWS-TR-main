...
spec:
  ...
  template: # bu kısımdan itibaren pod ve container detayları tanımlanır
    ...
    spec:
      containers:
      - name: web-flask-cont
        image: eagle79/wep-flask # docker hub repodan çekilen imajın adı
        ports:
        - containerPort: 5000 # container içindeki uygulamanın portu
```
...
spec:
  ...
  - port: 3000  # clusterIP servis portu
    targetPort: 5000 # container yayınını yakalamak için hedef port
  selector:
...
- Create a `my-db.yaml` file to create a Pod that connects the web-flask page via curl command.
...
spec:
  containers:
  - name: my-db
    image: eagle79/web-db
    imagePullPolicy: IfNotPresent
  restartPolicy: Always
```

- Create the `my-db` pod and log into the container. Connect the web-flask page via curl command and pay attention to load balancing.
